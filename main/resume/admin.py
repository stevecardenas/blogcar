...
class SkillAdmin(admin.ModelAdmin):
    list_display = ['name', 'category', 'proficiency', 'get_proficiency_display', 'get_icon_display', 'order']
    ...
    fieldsets = (
        ...
        ('Icona', {
            'fields': ('icon',),
            'description': 'Lascia vuoto per usare l\'icona automatica basata sul nome e categoria'
        }),
        ('Ordinamento', {
            'fields': ('order',),
            'classes': ('collapse',)
    ...
    def get_icon_display(self, obj):
        icon = obj.get_icon()
        return f'<i class="bi {icon}"></i> {icon}'
    get_icon_display.short_description = 'Icona'
